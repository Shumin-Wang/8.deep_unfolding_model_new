...
def generate_constrainsChannleAll(dd, D2, Nr, Nris, K, ob_down,ob_up,lambdaA,rx_antenna_gain):
    ...
    constant = np.sqrt(lambdaA**4/(256*np.pi**2))
    s_ris = lambdaA/2
    ...
    for iii in range(len(theta01)):
        ...
        G3 = generateChannelRXRISForConstrain(rx_arr,Nris,s_ris,Nr,constant,rx_antenna_gain,lambdaA,K)
        # [Hdir3, U, G3] = chan_mat_RIS_new_model_test(Nt, Nr, Nris, lt, z_matrix1[iii], D, 1, K, f, dist_ris, rx_arr)
        G_list[iii, :, :] = G3

    return G_list

# def generate_constrainsChannleAll(dd, D2, Nt, Nr, Nris, lt, D, K, f, dist_ris,ob_down,ob_up,lambdaA,rx_antenna_gain):
#     [phi01, theta01] = np.meshgrid(0, np.arange(ob_down, ob_up + dd, dd))
#     z_matrix1 = D2 * np.cos(theta01 / 180 * np.pi)  # convert to 3D Cartesian coordinate system (position matrix of Rx antenna)
#     x_matrix1 = D2 * np.sin(theta01 / 180 * np.pi) * np.cos(phi01 / 180 * np.pi)
#     y_matrix1 = D2 * np.sin(theta01 / 180 * np.pi) * np.sin(phi01 / 180 * np.pi)
...
def generatePositionRISArr(N_ris, s_ris):

    ris_arr = np.zeros([3, N_ris])
    for i in range(int(np.sqrt(N_ris))):
        for ii in range(int(np.sqrt(N_ris))):
            ris_arr[0, (i) * int(np.sqrt(N_ris)) + ii] = (i+1 - (np.sqrt(N_ris) + 1) / 2) * s_ris
            ris_arr[1, (i ) * int(np.sqrt(N_ris)) + ii] = (ii+1 - (np.sqrt(N_ris) + 1) / 2) * s_ris

    return ris_arr


def generateModel(HLos,K):

    HNLos = np.sqrt(1/2)*(np.random.randn(*HLos.shape)+1j*np.random.randn(*HLos.shape))
    H = (1/np.sqrt(K+1))*(np.sqrt(K)*HLos+HNLos*HLos)
    return H

def generateChannelRXRISForConstrain(rx_arr,N_ris, s_ris, N_r, constant, rx_antenna_gain, lambdaA,K):
    ris_arr = generatePositionRISArr(N_ris, s_ris)
    Hr_rx_los = np.zeros([N_r, N_ris],dtype=np.complex128)

    for i in range(N_r):
        for ii in range(N_ris):
            distance_nr_nm = np.linalg.norm(rx_arr[:, i] - ris_arr[:, ii])
            cos_theta_r_nr_nm = rx_arr[2, i] / distance_nr_nm
            Amplitude_nr_nm = constant * rx_antenna_gain * cos_theta_r_nr_nm / distance_nr_nm / distance_nr_nm
            Phase_term_nr_nm = np.exp(1j * 2 * np.pi * distance_nr_nm / lambdaA )
            Hr_rx_los[i, ii] = np.sqrt(Amplitude_nr_nm) * Phase_term_nr_nm


    Hr_rx = generateModel(Hr_rx_los,K)
...
def generate_constrains_mid(F1, F2, dd, D2, Nr, Nris, K, ob_down, ob_up,U,lambdaA,rx_antenna_gain):

    F1 = F1.detach().numpy()
    F2 = F2.detach().numpy()

    G_list= generate_constrainsChannleAll(dd, D2, Nr, Nris, K, ob_down,ob_up,lambdaA,rx_antenna_gain)
...
def generateRPDbm(theta,F1,F2,dd, D2, Nt, Nr, Nris, lt, D, K, f, dist_ris, U):
            allList = generate_constrains_mid(F1, F2, dd, D2, Nt, Nr, Nris, lt, D, K, f, dist_ris, -89, 89, U)
            num_g = allList.shape[0]
            Theta = theta@torch.conj(theta).T
            dbm_value = torch.zeros([1,num_g])
            for f_i in range(num_g):
                pow_value = torch.real(torch.trace(Theta@allList[f_i,:,:]))
                dbm_value[f_i] = 10*torch.log10(1000*pow_value)

            return dbm_value


def figureRB(db_value):
    theta_phi = np.arange(-89,89.5,0.5)
    plt.plot(theta_phi,db_value)
    plt.show()
